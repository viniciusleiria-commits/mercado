...
window.MERCADO_VERSAO = "10/09 11:07";
(function () {
...
(function () {
  ...
  function listaMarcavel(arr) {
    ...
    arr.forEach(function (it) {
      ...
      var m = marcado(it.id) || {};
      // Marcou, aparece o quanto ao lado — e não antes: linha não marcada é
      // para achar o item. O botão abre o mesmo teclado numérico da tela dela.
      var quanto = quer
        ? '<button class="qtd-btn' + (m.qtd || m.naoSei ? "" : " vazio") +
          (S.quantoAberto === it.id ? " aberto" : "") + '" type="button" ' +
          'data-acao="quanto" data-id="' + it.id + '">' +
          (m.qtd ? esc(m.qtd) : m.naoSei ? "não sei" : "quanto?") + "</button>"
        : "";
      // Números na mão em vez de teclado: um toque resolve a conta do dia a
      // dia. Acima de nove é raro (vinte ovos), e aí abre o campo para digitar.
      var teclas = "";
      if (quer && S.quantoAberto === it.id) {
        ...
        teclas += '<button class="tecla mais" type="button" data-acao="quanto-mais" data-id="' + it.id + '">9+</button>';
        teclas += "</div>";
      }
  ...
  document.getElementById("app").addEventListener("click", function (ev) {
    ...
    switch (a.acao) {
      ...
      case "quanto":
        S.quantoAberto = S.quantoAberto === a.id ? "" : a.id;
        desenhar();
        break;
      case "quanto-num":
        marcar(a.id, true, Number(a.valor));
        S.quantoAberto = "";
        break;
      case "quanto-mais": S.quantoAberto = ""; modalQuanto(a.id); break;
      case "convidar": {
        var senhaGuardada = "";
