...
window.MERCADO_VERSAO = "10/09 11:01";
(function () {
...
(function () {
  ...
  function listaMarcavel(arr) {
    ...
    arr.forEach(function (it) {
      ...
      var quanto = quer
        ? '<button class="qtd-btn' + (m.qtd || m.naoSei ? "" : " vazio") + '" type="button" ' +
  ...
  function modalQuanto(id) {
    ...
    abrirModal(
      ...
      'pattern="[0-9]*" autocomplete="off" value="' + esc(m.qtd ? m.qtd : "") + '" placeholder="0">' +
      '<div class="acoes-fim"><div class="linha-btns">' +
      '<button class="btn principal" type="button" data-acao="quanto-item-ok" data-id="' + id + '">Pronto</button>' +
      '<button class="btn" type="button" data-acao="quanto-item-nao-sei" data-id="' + id + '">Não sei</button>' +
      "</div></div></div></div>"
    );
  ...
  document.getElementById("app").addEventListener("click", function (ev) {
    ...
    switch (a.acao) {
      ...
      case "quanto": modalQuanto(a.id); break;
      case "convidar": {
        var senhaGuardada = "";
