...
window.MERCADO_VERSAO = "11/09 10:05";
(function () {
...
(function () {
  ...
  function desenhaRodada() {
    ...
    // O que ela marcou sem dizer quanto ("não sei", ou marcado antes do
    // teclado existir): é o que sobra para ele decidir. O que ela marcou com
    // número já está resolvido e aparece na lista abaixo, com o número.
    var daCasa = arr.filter(function (it) {
      var m = marcado(it.id);
      return m && m.precisa && m.por === "casa" && !m.qtd;
    });
    var aviso = "";
    if (daCasa.length) {
      aviso = '<div class="card"><h2>Falta confirmar a quantidade<span class="cont">' +
      ...
      daCasa.forEach(function (it) {
        ...
        aviso += '<div class="fila marcado"><span class="caixa">' + CHECK + "</span>" +
          ...
          '<span class="meta' + (m.naoSei ? " nao-sabe" : "") + '">' +
          esc(m.naoSei ? "ela não sabe" : dataCurta(m.quando || "")) + "</span></div>";
      });
      aviso += "</div>";
    }
  ...
  function listaMarcavel(arr) {
    ...
    arr.forEach(function (it) {
      ...
      // Escolhido o número, a fileira fecha e sobra só ele, cinza na beirada,
      // do mesmo jeito que a tela Listas mostra — foi o desenho que ele
      // escolheu em 10/09/2026, depois de ver os dois lado a lado.
  ...
  function desenhaPergunta() {
    ...
    // Ao abrir, começa de onde falta responder, não do primeiro item da lista.
    // O "← Voltar" continua ali para ela rever o que já respondeu.
    if (!S.filaPosicionada && fila.length) {
      S.filaPosicionada = true;
      var inicio = proximaSemResposta(0);
      S.perguntaIdx = inicio < 0 ? fila.length : inicio;
    }
    ...
    if (S.perguntaIdx >= fila.length) {
      ...
      if (faltam) {
        resumo += '<div class="corpo"><p class="sub" style="margin-bottom:8px">Ainda ' +
          (faltam === 1 ? "falta 1 item sem resposta" : "faltam " + faltam + " itens sem resposta") +
          " — eles entraram na lista depois.</p>" +
          '<button class="btn principal larga" type="button" data-acao="pergunta-faltam">' +
          "Ver os que faltam</button></div>";
      }
    ...
    alvo.innerHTML = topo +
      ...
      esc(aviso || (jaRespondeu
        ? (m.precisa
            ? "você marcou" + (m.qtd ? ": " + m.qtd : m.naoSei ? ": não sei" : "")
            : "você marcou: não precisa")
        : "")) +
      "</div></div>" +
      corpo +
      '<div class="pe">' +
  ...
  // A fila é percorrida por posição, e a posição só andava para a frente.
  // Duas consequências que ele viu em 10/09/2026: ao abrir, ela caía sempre no
  // primeiro item da lista (já respondido) em vez de onde parou; e item que ele
  ...
  async function novaRodada() {
    ...
    S.rodada = nova;
    S.tela = "rodada";
    // lista nova, fila dela do começo
    S.perguntaIdx = 0;
    S.perguntando = null;
    S.filaPosicionada = true;
    desenhar();
  ...
  document.getElementById("app").addEventListener("click", function (ev) {
    ...
    switch (a.acao) {
      ...
      case "pergunta-recomecar":
        ...
        break;
      case "pergunta-faltam": {
        var falta = proximaSemResposta(0);
        if (falta >= 0) { S.perguntaIdx = falta; S.perguntando = null; desenhar(); }
        break;
      }
      case "alternar":
